...
namespace sgraph
{
    ...
    public:
        ...
        std::map<std::string, std::string> getImagesMap() const
        {
            return imagesMap;
        }
    ...
    protected:
        ...
        virtual void parseLight(std::istream &input)
        {
            ...
            lightsMap[lightName] = light;
        }

        virtual void parseAssignLight(std::istream &input)
        {
            std::string lightName, nodeName;
            input >> lightName >> nodeName;
            if (nodes.find(nodeName) != nodes.end() && lightsMap.find(lightName) != lightsMap.end())
            {
                sgraph::AbstractSGNode *absNode = dynamic_cast<sgraph::AbstractSGNode *>(nodes[nodeName]);
                if (absNode)
                {
                    absNode->addLight(lightsMap[lightName]);
                }
            }
        }

        virtual void parseAssignTexture(std::istream &input)
        {
            std::string nodeName, imageName;
            input >> nodeName >> imageName;
            sgraph::LeafNode *leaf = dynamic_cast<sgraph::LeafNode *>(nodes[nodeName]);
            if (leaf && imagesMap.find(imageName) != imagesMap.end())
            {
                leaf->setTexture(imageName);
            }
        }

        virtual void parseImage(std::istream &input)
        {
            std::string imageName, imagePath;
            if (!(input >> imageName >> imagePath))
            {
                throw std::runtime_error("Failed to read image command arguments");
            }
            imagesMap[imageName] = imagePath;
            std::cout << "Read image: " << imageName << " " << imagePath << std::endl;
        }
    ...
    private:
        ...
        map<string, util::Light> lightsMap;
